...
public class InvocableHandlerMethod extends HandlerMethod {
	...
	protected Object[] getMethodArgumentValues(NativeWebRequest request, @Nullable ModelAndViewContainer mavContainer,
		...
		for (int i = 0; i < parameters.length; i++) {
			...
			// providedArgs是调用方提供的参数，默认调用方提供的参数都是长度为0的数组
			args[i] = findProvidedArgument(parameter, providedArgs);
			if (args[i] != null) {
				continue;
			}
			// 调用方提供的参数中不能找到当前类型的参数值，则遍历Spring容器中所有ArgumentResolver，
			// 判断哪种支持对当前参数的解析，判断标准是是否使用@@RequestParam进行标注
			if (!this.resolvers.supportsParameter(parameter)) {
				throw new IllegalStateException(formatArgumentError(parameter, "No suitable resolver"));
			}
			try {
				// 如果能找到对当前参数进行处理的ArgumentResolver，则调用resolveArgument()方法从request中获取相对应的参数进行转换
